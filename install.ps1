...
if (Test-Path -Path $VENV_DIR -PathType Container) {
    ...
    try {
        ...
        echo "py -$PYTHON_VERSION -m venv $VENV_DIR"
...
if (-not (Test-Path -Path $REQUIREMENTS_FILE)) {
    ...
    $venvActivateScript = Join-Path $VENV_DIR "Scripts\activate.bat"
    ...
    Write-Host "Installing nltk stopwords and punkt tokenizer..."
    try {
        python -m nltk.downloader stopwords punkt_tab
        Write-Host "NLTK data downloaded successfully." -ForegroundColor Green
    }
    catch {
        Write-Host "Error: Failed to download NLTK data." -ForegroundColor Red
